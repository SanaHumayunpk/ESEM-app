...
export const QuestionnaireFlow: React.FC<QuestionnaireFlowProps> = ({
  ...
  const isComplete = Boolean(skinFeel && mainConcern && sensitivity);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!skinFeel || !mainConcern || !sensitivity) return;
      ...
      <div className="bg-white rounded-2xl border border-rose-100/80 p-6 sm:p-8 shadow-xs space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="pt-4 space-y-2">
            ...
            {!isComplete && (
              <p className="text-[11px] text-amber-700 text-center font-medium bg-amber-50/80 py-1.5 px-3 rounded-lg border border-amber-200/60">
                Please select an option for questions 1, 2, and 3 to continue.
              </p>
            )}
          </div>
        </form>
